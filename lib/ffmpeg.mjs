 ...
 */
export async function stillImageToVideo({ imagePath, audioPath, duration, outputPath }) {
  ...
  if (audioPath) {
    ...
    cmd = [
      ...
      `-c:a aac -b:a 192k`,
      `-shortest`,
      `-vf "scale=8000x4500,zoompan=z='min(zoom+0.0015,1.5)':d=125:x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)',scale=1920x1080"`,
      `"${outputPath}"`,
    ].join(' ');
  } else if (duration) {
