...
export async function generateEditPlan({ script, sceneMeasurements, sceneExtraClips = {}, tmpDir }) {
  // STEP 1 — Generate shot plans for ALL scenes via a single batched Claude call
  const edl = { scenes: [] };

  // Pair each measurement with its script scene and extra clips
  const sceneInputs = [];
  for (const measurement of sceneMeasurements) {
    ...
    sceneInputs.push({ scene, measurement, extraClips });
  }

  // Try batched call first, fall back to per-scene on failure
  const batchedPlans = await generateBatchedShotPlans(sceneInputs);

  for (const { scene, measurement, extraClips } of sceneInputs) {
    let shotPlan = batchedPlans[measurement.sceneNumber];

    // Fallback: if batch didn't produce a valid plan for this scene, retry individually
    if (!shotPlan) {
      console.warn(`  ⚠️  Scene ${measurement.sceneNumber}: batch missing — retrying individually`);
      shotPlan = await generateShotPlan({ scene, measurement, extraClips });
    }
...
/**
 * Batch all scene shot plans into a single Claude call.
 * Returns { [sceneNumber]: shots[] } — missing keys mean that scene needs individual retry.
 */
async function generateBatchedShotPlans(sceneInputs) {
  const results = {};
  if (sceneInputs.length === 0) return results;

  // Build per-scene descriptions for the prompt
  const sceneBlocks = sceneInputs.map(({ scene, measurement, extraClips }) => {
    ...
    if (extraClips.length > 0) {
      ...
      extraClipsInfo = `\n  Extra Hailuo clips (${extraClips.length}, ~10s each — use AFTER primary animation):\n${clipList}`;
    }

    return `--- Scene ${sceneNumber} (${sceneType}) ---
  Description: "${sceneDesc}"
  Audio duration: ${audioDuration.toFixed(1)}s
  Animation duration: ${animDuration.toFixed(1)}s
  Has animation: ${!!animPath}${animPath ? ` (sourcePath: "${animPath}")` : ''}
  Has base image: ${!!imagePath}${imagePath ? ` (sourcePath: "${imagePath}")` : ''}${extraClipsInfo}`;
  }).join('\n\n');

  const systemPrompt = `You are a video editor planning shots for a children's animated Tamil village story.
You will receive details for MULTIPLE scenes and must return shot plans for ALL of them in a single JSON response.

Rules (apply to EACH scene independently):
- Shots for each scene must cover exactly that scene's audio duration
...
Shot types:
- "animation": uses the pre-rendered scene animation. Set sourcePath to the animation path listed for that scene. If no animation exists, use "still" instead.
- "filler": a new image generated to fill time. Provide an imagenPrompt describing the filler visual.
- "still": uses the scene's base image as a static shot. Set sourcePath to the base image path.
...
Return ONLY a valid JSON object with scene numbers as keys mapping to shot arrays:
{ "scene_1": [...shots], "scene_2": [...shots], ... }`;

  const userPrompt = `Plan shots for all ${sceneInputs.length} scenes below:\n\n${sceneBlocks}\n\nReturn the combined shot plans as a JSON object keyed by scene number (e.g. "scene_1", "scene_2").`;
  ...
  try {
    ...
    if (!jsonMatch) throw new Error('No JSON object found in batched response');

    const parsed = JSON.parse(jsonMatch[0]);

    // Extract and validate each scene's shots
    for (const { measurement } of sceneInputs) {
      const key = `scene_${measurement.sceneNumber}`;
      const shots = parsed[key];
      if (!Array.isArray(shots) || shots.length === 0) {
        console.warn(`  ⚠️  Scene ${measurement.sceneNumber}: invalid or missing in batch response`);
        continue;
      }

      // Validate total duration
      const totalDuration = shots.reduce((sum, s) => sum + s.duration, 0);
      if (Math.abs(totalDuration - measurement.audioDuration) > 1.0) {
        console.warn(`  ⚠️  Scene ${measurement.sceneNumber}: shot total ${totalDuration.toFixed(1)}s vs audio ${measurement.audioDuration.toFixed(1)}s — adjusting last shot`);
        const diff = measurement.audioDuration - totalDuration;
        shots[shots.length - 1].duration += diff;
      }

      console.log(`  🎬 Scene ${measurement.sceneNumber}: ${shots.length} shots planned (batch)`);
      results[measurement.sceneNumber] = shots;
    }
  } catch (err) {
    console.warn(`  ⚠️  Batched shot planning failed: ${err.message} — will retry scenes individually`);
  }
 ...
 */
async function generateShotPlan({ scene, measurement, extraClips = [] }) {
  const { sceneNumber, audioDuration, animDuration, animPath, imagePath } = measurement;
