...
export async function insertConcept({ title, theme, synopsis, characters, outline, art_style, video_type, teasers_enabled = false }) {
  const sb = getSupabase();
  const { data, error } = await sb
    .from('concepts')
    .insert({ title, theme, synopsis, characters, outline, art_style, video_type, teasers_enabled })
    .select('id')
    .single();
...
export async function insertTeaser(taskId, { teaser_number, archetype, scene_numbers, title_text, hook_text, teaser_last_scene_dialogue, narrator_outro_tamil }) {
  const sb = getSupabase();
  const { data, error } = await sb
    .from('teasers')
    .insert({
    ...
    .select('id')
    .single();
  if (error) throw new Error(`insertTeaser(${teaser_number}) failed: ${error.message}`);
  return data.id;
}

export async function getTeasers(taskId) {
  const sb = getSupabase();
  const { data, error } = await sb
    .from('teasers')
    .select('*')
    .eq('task_id', taskId)
    .order('teaser_number', { ascending: true });
  if (error) throw new Error(`getTeasers failed: ${error.message}`);
  return data || [];
}

export async function updateTeaser(taskId, teaserNumber, fields) {
  const sb = getSupabase();
  const { error } = await sb
    .from('teasers')
    .update(fields)
    .eq('task_id', taskId)
    .eq('teaser_number', teaserNumber);
  if (error) throw new Error(`updateTeaser(${teaserNumber}) failed: ${error.message}`);
}
