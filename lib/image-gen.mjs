 ...
 */
async function callGeminiImageAPI({ prompt, aspectRatio = '16:9', resolution = '1K', apiKey }) {
 ...
 */
export async function generateSceneImage({ prompt, sceneNumber = 0, aspectRatio = '16:9', resolution = '1K' }) {
  const apiKey = process.env.GOOGLE_AI_API_KEY;
  if (!apiKey) throw new Error('GOOGLE_AI_API_KEY not set');

  // Attempt: Gemini 3.1 Flash Image
  try {
    return await callGeminiImageAPI({ prompt, aspectRatio, resolution, apiKey });
  } catch (err) {
    if (err.is429) {
