...
async function improveCharacterPrompt(currentPrompt, feedbackArray, characterName) {
  ...
  try {
    ...
    const message = await withRateLimit(() => client.messages.create({
      ...
      max_tokens: 1024,
      system: [{ type: 'text', text: 'You are improving image generation prompts for Tamil kids animated characters. Given a current prompt and human feedback, produce an improved prompt that addresses the feedback. Return ONLY the improved prompt text, no explanation.', cache_control: { type: 'ephemeral' } }],
      messages: [{
        role: 'user',
