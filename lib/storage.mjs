 ...
 * @param {string} [opts.variant] - 'long' (default, 16:9) or 'teaser' (9:16)
 */
export async function uploadSceneImage({ videoId, sceneNumber, buffer, variant = 'long' }) {
  const suffix = variant === 'teaser' ? '_teaser' : '';
  const path = `${videoId}/scene_${String(sceneNumber).padStart(2, '0')}_image${suffix}.png`;
 ...
 * @param {string} [opts.variant] - 'long' (default, 16:9) or 'teaser' (9:16)
 */
export async function uploadSceneAnimation({ videoId, sceneNumber, buffer, variant = 'long' }) {
  const suffix = variant === 'teaser' ? '_teaser' : '';
  const path = `${videoId}/scene_${String(sceneNumber).padStart(2, '0')}_anim${suffix}.mp4`;
  return uploadToStorage({ bucket: BUCKETS.scenes, path, buffer, contentType: 'video/mp4' });
}

/**
 * Upload scene audio (MP3) to Supabase Storage.
 */
export async function uploadSceneAudio({ videoId, sceneNumber, buffer }) {
  const path = `${videoId}/scene_${String(sceneNumber).padStart(2, '0')}_audio.mp3`;
  return uploadToStorage({ bucket: BUCKETS.scenes, path, buffer, contentType: 'audio/mpeg' });
}
