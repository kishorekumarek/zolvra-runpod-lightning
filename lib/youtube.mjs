...
/**
 * Upload a video as Private for human review.
 * Returns the YouTube video ID.
 */
export async function uploadVideoPrivate({ videoPath, script, taskId }) {
  ...
  console.log(`📺 Uploaded to YouTube (private): https://youtu.be/${videoId}`);
  return videoId;
}
...
export async function publishVideo({ youtubeVideoId, scheduleAt = null }) {
  ...
  const status = {
    privacyStatus:           'private',  // Always private — Darl reviews before making public
  ...
  console.log(
    scheduleAt
      ...
      : `🌍 Published YouTube video ${youtubeVideoId} (private — awaiting Darl approval)`
  );
}
